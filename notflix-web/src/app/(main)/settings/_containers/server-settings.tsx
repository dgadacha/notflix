...
export function ServerSettings(props: ServerSettingsProps) {
    ...
    const confirmDialog = useConfirmationDialog({
        ...
        description: "This will upload your local Notflix collection to your AniList account. Are you sure you want to proceed?",
        actionText: "Upload",
        ...
        <div className="space-y-4">
            ...
            <SettingsCard>
                ...
                <Field.Switch
                    ...
                    help="If enabled, Notflix will remember your watch progress and resume from where you left off."
                    moreHelp="Only applies to desktop and integrated players."
                />
            ...
            <SettingsCard title="Offline mode" description="Only available when authenticated with AniList.">
                ...
                <Field.Switch
                    ...
                    help="If enabled, Notflix will automatically save all media you're currently watching/reading for offline use."
                />
            ...
            <SettingsCard title="App">
                ...
                <Field.Switch
                    ...
                    moreHelp="Notifications shown by the OS when Notflix runs the auto-downloader or auto-scanner."
                />
                <Field.Switch
                    side="right"
                    name="disableCacheLayer"
                    label="Disable AniList caching"
                    help="If enabled, Notflix will stop caching AniList requests to disk."
                    moreHelp="By default, all requests made to AniList are cached. This allows Notflix to keep being usable when AniList goes down. The cache directory is modifiable in the config file."
                />
                {!f.watch("disableCacheLayer") && (
                    <div>
                        <Switch
                            ...
                            moreHelp="Notflix will use cached data instead of making API requests."
                        />
                    </div>
                )}
                <Field.Switch
                    ...
                    help="If enabled, Notflix will use an alternative source to fetch episode metadata."
                />
                {/*<Separator />*/}
                {/*<Field.Switch*/}
                {/*    side="right"*/}
                ...
                <Field.Switch
                    ...
                    help={__isElectronDesktop__ ? (<span className="flex gap-2 items-center">
                        ...
                        <span>If enabled, new releases won't be displayed. Notflix Denshi may still auto-update in the background.</span>
                    </span>) : "If enabled, Notflix will not check for new releases."}
                    moreHelp={__isElectronDesktop__ ? "You cannot disable auto-updates for Notflix Denshi." : undefined}
                />
                <Field.Select
                    label="Update Channel (Experimental)"
                    name="updateChannel"
                    help={__isElectronDesktop__ ? "Also applies to Notflix Denshi auto-updates." : ""}
                    options={[
                        { label: "GitHub (Default)", value: "github" },
                        { label: "Notflix", value: "seanime" },
                        { label: "Notflix (Canary)", value: "seanime_nightly" },
                    ]}
                />
                {serverStatus?.settings?.library?.updateChannel === "seanime" && (
                    <Alert intent="info" description="You are currently using a release channel hosted on Notflix." />
                )}
                {serverStatus?.settings?.library?.updateChannel === "seanime_nightly" && (
                    <Alert
                        intent="warning"
                        description="You are currently using the canary release channel hosted on Notflix. This channel may receive unstable updates without much testing."
                    />
                )}
            </SettingsCard>
