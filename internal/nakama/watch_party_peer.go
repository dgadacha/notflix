...
func (wpm *WatchPartyManager) handleWatchPartyStateChangedEvent(payload *WatchPartyStateChangedPayload) {
	...
	if (newPlayback || playbackChanged || peerJoined) &&
		...
		case WatchPartyStreamTypeDebrid:
			...
			err = wpm.manager.PlayHostAnimeStream(payload.Session.CurrentMediaInfo.StreamType, "notflix/nakama", wpm.clientId, media, payload.Session.CurrentMediaInfo.AniDBEpisode)
		case WatchPartyStreamTypeFile:
			// Start the local file stream off of the host using the file path
			err = wpm.manager.PlayHostAnimeLibraryFile(payload.Session.CurrentMediaInfo.LocalFilePath, "notflix/nakama", wpm.clientId, media, payload.Session.CurrentMediaInfo.AniDBEpisode, "")
